...
bool OnEventGameEnd(TableFrame *table, int wChairID, uint8_t cbReason)
{
	...
	case GER_NORMAL: // 常规结束
	{
		...
		for (int i = 0; i < table->m_wPlayerCount; i++)
		{
			gameEnd.bCardCount[i] = table->m_bCardCount[i];
			memcpy(&gameEnd.bCardData[bCardPos], &table->m_bHandCardData[i], table->m_bCardCount[i] * sizeof(uint8_t));
			bCardPos += table->m_bCardCount[i];
		}

		// 变量定义
		// long lCellScore = table->m_pGameServiceOption->lCellScore;
		long lCellScore = 10000;
		...
		for (int i = 0; i < table->m_wPlayerCount; i++)
		{
			if (i == table->m_wBankerUser)
				gameEnd.lGameScore[i] = table->m_wBombTime * table->m_bLandScore * lCellScore * ((bLandWin == true) ? 2 : -2);
			else
				gameEnd.lGameScore[i] = table->m_wBombTime * table->m_bLandScore * lCellScore * ((bLandWin == true) ? -1 : 1);
		}
...
bool OnUserLandScore(TableFrame *table, int wChairID, uint8_t bLandScore)
{
	...
	if ((table->m_bLandScore == 3) || (table->m_wFirstUser == (wChairID + 1) % table->m_wPlayerCount))
	{
		...
		// 发送消息
		// GameStart GameStart;
		// GameStart.wLandUser = table->m_wBankerUser;
		// GameStart.bLandScore = table->m_bLandScore;
		// GameStart.wCurrentUser = table->m_wCurrentUser;
		// memcpy(GameStart.bBackCard, table->m_bBackCard, sizeof(table->m_bBackCard));
		// table->m_pITableFrame->SendTableData(-1, SUB_S_GAME_START, &GameStart, sizeof(GameStart));
		// table->m_pITableFrame->SendLookonData(-1, SUB_S_GAME_START, &GameStart, sizeof(GameStart));
	...
	// m_pITableFrame->SendTableData(-1, SUB_S_LAND_SCORE, &LandScore, sizeof(LandScore));
	// m_pITableFrame->SendLookonData(-1, SUB_S_LAND_SCORE, &LandScore, sizeof(LandScore));
...
bool OnUserOutCard(TableFrame *table, int wChairID, uint8_t bCardData[], uint8_t bCardCount)
{
	...
	if (bCardType == CT_MISSILE_CARD)
		table->m_bTurnCardCount = 0;
	// my_card_to_palette(wChairID, table->m_bHandCardData, table->m_bCardCount[wChairID]);
	// palette_to_screen();
	// 结束判断
	// if (table->m_wCurrentUser == -1)
